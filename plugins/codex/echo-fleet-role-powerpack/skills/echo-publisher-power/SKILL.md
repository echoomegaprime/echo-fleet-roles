...
description: Package, document, release, and prove ECHO artifacts without altering product logic. Use for cauto publisher sessions, repository hygiene, release preparation, documentation, or delivery evidence.
---
...
Load `$echo-delivery-evidence`, `$echo-proofline-verification`, and `$github:github`. Load `$docx`, `$pdf`, or `$pptx` only when the artifact format requires it.
